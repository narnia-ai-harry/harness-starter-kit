...
4. **Identify the original repo path**: Use `git rev-parse --git-common-dir` to find the main repo's git directory, then derive the original repo working directory (its parent).
...
3. **Handle conflicts**: If the merge reports conflicts:
   ...
   - The WIP commit history from Phase 2 provides context for understanding *why* changes were made, which helps the user resolve conflicts more effectively
...
2. **Report summary**: Tell the user:
   - The final commit hash and summary line
...
- **Never skip pre-commit hooks** (`--no-verify`) during the final squash merge commit.
...
Based on the merge-worktree skill from [codefactory-co/kimoring-ai-skills](https://github.com/codefactory-co/kimoring-ai-skills).
